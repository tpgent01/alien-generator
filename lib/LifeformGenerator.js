...
class LifeformGenerator {
    ...
    initializeLifeformGenerator() {
        ...
        this.lifeform.backstory = `${this.lifeform.name} ${this.generateBirth()}.`
...
"lifeform": ${lifeform}
    ...
    generateBirth() {   
        ...
        if (chance <= 25 && chance > 0) {
            return `is the first born of ${random.dice(6,3).reduce((a, b) => a + b, 0)} genetically related lifeforms`
        } else if (chance <= 35 && chance > 25) {
            return `was just one of a litter of ${random.dice(100,3).reduce((a, b) => a + b, 0)} offspring`
        } else if (chance <= 45 && chance > 35) {
            return `was hatched after an incubation period of ${random.dice(3,20).reduce((a, b) => a + b, 0)} years from a nest of ${random.dice(10,2).reduce((a, b) => a + b, 0)} eggs`
        } else if (chance <= 55 && chance > 45) {
            return `was the only survivor of ${random.dice(100,3).reduce((a, b) => a + b, 0)} offspring, the rest were consumed by its parents`
        } else if (chance <= 75 && chance > 55) {
            return `is just one of its parents ${random.die(900)},000 pupa that emerged from a pupal case`
            ...
            return `is the last born of ${random.dice(6,3).reduce((a, b) => a + b, 0)} genetically related lifeforms`
        } else {
