...
class LifeformGenerator {
    ...
    initializeLifeformGenerator() {
        ...
        this.lifeform.backstory = `${this.lifeform.name} ${this.generateDisposition()} It ${this.generateBirth()}. ${this.lifeform.name} ${this.generateReason()}`
        ...
        this.lifeform.physicalDescription = `${this.lifeform.name} is ${this.lifeform.locamotion} that ${this.generateUpperMadLib()} using its ${this.lifeform.upperAppendageType}. ${this.lifeform.name}${this.lifeform.head} ${this.lifeform.facialFeatures}. ${this.generateFaceMadLib()} its ${this.lifeform.eyes} eyes, ${this.lifeform.nose}, ${this.lifeform.ears} ears, and ${this.lifeform.mouth}! ${this.lifeform.name}'s epidermis is ${this.lifeform.skin}, but most notably ${this.lifeform.name} has ${this.lifeform.otherFeatures}.`;
...
"lifeform": ${lifeform}
    ...
    generateLocomotion() {
        ...
        if (chance ===2) { return `${this.generateOverallProportions()} Creeper (Hexapedal Locomotion)` }
        else if (chance === 3) { return `${this.generateOverallProportions()} Plopper (Gelatinous Locomotion)` }
        else if (chance === 4) { return `${this.generateOverallProportions()} Squirmer (Tentacled Octopedal Locomotion)` }
        else if (chance === 5) { return `${this.generateOverallProportions()} Hopper (Bipedal Jumping Locomotion)` }
        else if (chance === 6) { return `${this.generateOverallProportions()} Clinger (Brachiation Locomotion)` }
        else if (chance === 7) { return `${this.generateOverallProportions()} Normy (Bipedal Walking Locomotion)` }
        else if (chance === 8) { return `${this.generateOverallProportions()} Knuckle Dragger (Orthograde Locomotion)` }
        else if (chance === 9) { return `${this.generateOverallProportions()} Flapper (Avian Locomotion)` }
        else if (chance === 10) { return `${this.generateOverallProportions()} Slitherer (Unipedal Locomotion)` }
        else if (chance === 11) { return `${this.generateOverallProportions()} Looper (Peristalsis and Looping Locomotion)` }
        else { return `${this.generateOverallProportions()} Trotter (Quadrupedal Locomotion)` }
    }

    generateOverallProportions() {
        let chance = random.die(6)
        if (chance === 1) { return `a Lanky` }
        else if (chance === 2) { return `an Obese` }
        else if (chance >= 3 && chance <= 5 ) { return `an Average sized` }
        else { return `a Muscular` }
    }
    ...
    generateUpperAppendageType() {
        ...
        else if (chance >= 3 && chance <= 5 ) { return `${this.generateUpperAppendageNumber()} ${this.generateLength()}Humanoid arms and ${this.generateSize()}humanoid hands. Each with an opposable thumb and ${this.generateFingerNumber()}` }
        else { return `${this.generateUpperAppendageNumber()} ${this.generateLength()}Multijointed (Insectoid/Arachnid-like) Upper Limbs. Each one with a surprisingly dexterous pincher at its end` }
    }

    generateUpperAppendageNumber() {
        let chance = random.die(6)
        if (chance >= 1 && chance <= 2 ) { return `four` }
        else if (chance >= 3 && chance <= 5 ) { return `two` }
        else { return `six` }
    }

    generateFingerNumber() {
        let chance = random.die(6)
        if (chance === 1) { return `a single ${this.generateLength()}finger` }
        else if (chance === 3 ) { return `three ${this.generateLength()}fingers` }
        else if (chance === 4 ) { return `four ${this.generateLength()}fingers` }
        else if (chance === 5 ) { return `five ${this.generateLength()}fingers` }
        else { return `six ${this.generateLength()}fingers` }
    }
    ...
    generateSkinType() {
        let chance = random.die(100)
        if (chance >= 1 && chance <= 4) { return `Mossy Flesh` }	
        else if (chance >= 5 && chance <= 8) { return `Decomposing Flesh` }	
        else if (chance >= 9 && chance <= 12) { return `Glowing Skin` }	
        else if (chance >= 13 && chance <= 16) { return `Bony Exoskeleton` }	
        else if (chance >= 17 && chance <= 20) { return `Foliage` }	
        else if (chance >= 21 && chance <= 24) { return `Lumpy Flesh` }	
        else if (chance >= 25 && chance <= 28) { return `Spectral Form` }	
        else if (chance >= 29 && chance <= 32) { return `Crystalized Exoskeleton` }	
        else if (chance >= 33 && chance <= 36) { return `Vapor Form` }	
        else if (chance >= 37 && chance <= 40) { return `Shimmering Skin` }	
        else if (chance >= 41 && chance <= 44) { return `Shark-like Flesh` }	
        else if (chance >= 45 && chance <= 48) { return `Leathery Hide` }	
        else if (chance >= 49 && chance <= 52) { return `Rind Flesh (Like an Orange)` }	
        else if (chance >= 53 && chance <= 56) { return `Translucent Skin` }	
        else if (chance >= 57 && chance <= 60) { return `Fur` }	
        else if (chance >= 61 && chance <= 64) { return `Scaled Skin` }	
        else if (chance >= 65 && chance <= 68) { return `Quills` }	
        else if (chance >= 69 && chance <= 72) { return `Feathers` }	
        else if (chance >= 73 && chance <= 76) { return `Flesh` }	
        else if (chance >= 77 && chance <= 80) { return `Sandpaper Textured Hide` }	
        else if (chance >= 81 && chance <= 84) { return `Rocky Hide` }	
        else if (chance >= 85 && chance <= 88) { return `Pinecone-like Exoskeleton` }	
        else if (chance >= 89 && chance <= 92) { return `Peach Fuzzed Textured Skin` }	
        else if (chance >= 93 && chance <= 96) { return `Slimy Flesh` }	
        else { return `Coral-like Exoskeleton` }	
    }
    ...
    generateSkinPattern() {
        ...
        else { return `${this.generateSkinColor()} ${this.generateSkinType()}` }
    }
    ...
    generateFeature() {
        ...
        else if (chance >= 36 && chance <= 40) { return `a ${this.generateLength()}Tail` }
        ...
        else if (chance >= 51 && chance <= 55) { return `a ${this.generateLength()}Prehensile Tongue` }
    ...
    generateUpperMadLib () {
        ...
        else if (chance >= 3 && chance <= 5 ) { return `manipulates its immediate environment` }
        else { return `imposes its will on surrounding objects and entities` }
    }
    ...
    generateAbility() {
        let chance = random.die(100)
        if (chance >= 1 && chance <=2) { return abilities.absorbEnergy }
        else if (chance >= 3 && chance <=4) { return abilities.acidBlood }
        else if (chance >= 5 && chance <=6) { return abilities.acidBreath }
        else if (chance >= 7 && chance <=8) { return abilities.cling }
        else if (chance >= 9 && chance <=10) { return abilities.gelatinousStorage }
        else if (chance >= 11 && chance <=12) { return abilities.gliding }
        else if (chance >= 13 && chance <=14) { return abilities.highJumping }
        else if (chance >= 15 && chance <=16) { return abilities.languageComprehension }
        else if (chance >= 17 && chance <=18) { return abilities.macroVision }
        else if (chance >= 19 && chance <=20) { return abilities.nightVision }
        else if (chance >= 21 && chance <=22) { return abilities.selfContainedBioFunctions }
        else if (chance >= 23 && chance <=24) { return abilities.telepathy }
        else if (chance >= 25 && chance <=26) { return abilities.thermalVision }
        else if (chance >= 27 && chance <=28) { return abilities.webbing }
        else if (chance >= 29 && chance <=30) { return abilities.acceleratedEvolution }
        else if (chance >= 31 && chance <=32) { return abilities.animalControl }
        else if (chance >= 33 && chance <=34) { return abilities.damageShield }
        else if (chance >= 35 && chance <=36) { return abilities.duplicationI }
        else if (chance >= 37 && chance <=38) { return abilities.empathy }
        else if (chance >= 39 && chance <=40) { return abilities.bindSenses }
        else if (chance >= 41 && chance <=42) { return abilities.extraLimbsCrafting }
        else if (chance >= 43 && chance <=44) { return abilities.highSpeedRunning }
        else if (chance >= 45 && chance <=46) { return abilities.mezmerize }
        else if (chance >= 47 && chance <=48) { return abilities.mindProbe }
        else if (chance >= 49 && chance <=50) { return abilities.naturalResistance }
        else if (chance >= 51 && chance <=52) { return abilities.poisonGas }
        else if (chance >= 53 && chance <=54) { return abilities.redistributeDamage }
        else if (chance >= 55 && chance <=56) { return abilities.regeneration }
        else if (chance >= 57 && chance <=58) { return abilities.telekinesis }
        else if (chance >= 59 && chance <=60) { return abilities.totalRecall }
        else if (chance >= 61 && chance <=62) { return abilities.venom }
        else if (chance >= 63 && chance <=64) { return abilities.changeling }
        else if (chance >= 65 && chance <=66) { return abilities.mindReading }
        else if (chance >= 67 && chance <=68) { return abilities.phaseShifting }
        else if (chance >= 69 && chance <=70) { return abilities.sizeControl }
        else if (chance >= 71 && chance <=72) { return abilities.split }
        else if (chance >= 73 && chance <=74) { return abilities.symbionts }
        else if (chance >= 75 && chance <=76) { return abilities.temporaryInvulnerability }
        else if (chance >= 77 && chance <=78) { return abilities.sixthSense }
        else if (chance >= 79 && chance <=80) { return abilities.fortitude }
        else if (chance >= 81 && chance <=82) { return abilities.might }
        else if (chance >= 83 && chance <=84) { return abilities.nimble }
        else if (chance >= 85 && chance <=86) { return abilities.beauty }
        else if (chance >= 87 && chance <=88) { return abilities.alienIntelligence }
        else if (chance >= 89 && chance <=90) { return abilities.duplicationIii }
        else if (chance >= 91 && chance <=92) { return abilities.elasticityIi }
        else if (chance >= 93 && chance <=94) { return abilities.flight }
        else if (chance >= 95 && chance <=96) { return abilities.illusionMajor }
        else if (chance >= 97 && chance <=98) { return abilities.invisibility }
        else { return abilities.mindControl }
    ...
    gernerateAbilities() {
        ...
        if (chance >= 1 && chance <= 3 ) {  
            abilitiesArr.push({name: abilityOne.name, description:abilityOne.description});
            abilitiesArr.push({name: abilityTwo.name, description:abilityTwo.description});
        }
        else {
            abilitiesArr.push({name: abilityOne.name, description:abilityOne.description});
            abilitiesArr.push({name: abilityTwo.name, description:abilityTwo.description});
            abilitiesArr.push({name: abilityThree.name, description:abilityThree.description});
        }
    ...
    generateBirth() {   
        let chance = random.die(100)
        if (chance >= 1 && chance <= 15) { return `is the first born of ${random.dice(6,3).reduce((a, b) => a + b, 0)} genetically related lifeforms`} 
        else if (chance >= 16 && chance <= 28) { return `was just one of a litter of ${random.dice(100,3).reduce((a, b) => a + b, 0)} offspring`} 
        else if (chance >= 29 && chance <= 43) { return `was hatched after an incubation period of ${random.dice(3,20).reduce((a, b) => a + b, 0)} years from a nest of ${random.dice(10,2).reduce((a, b) => a + b, 0)} eggs`} 
        else if (chance >= 44 && chance <= 57) { return `was the only survivor of ${random.dice(100,3).reduce((a, b) => a + b, 0)} offspring, the rest were consumed by its parents`} 
        else if (chance >= 58 && chance <= 70) { return `is just one of its parents ${random.die(900)},000 pupa that emerged from a pupal case`} 
        else if (chance >= 71 && chance <= 80) { return `is a conjoined twin that consumed its sibling during puberty`} 
        else if (chance >= 81 && chance <= 90) { return `is the last born of ${random.dice(6,3).reduce((a, b) => a + b, 0)} genetically related lifeforms`} 
        else { return `is an illegitimate bastard offspring of inter-dimensional gypsies`} 
    }

    generateReason() {
        let chance = random.die(100)
        if (chance >= 1 && chance <= 10) { return `is an outlaw wanted in over ${random.dice(10,3).reduce((a, b) => a + b, 0)} dimensions.`}
        else if (chance >= 11 && chance <= 20) { return `is on a quest for power and glory by way of ${this.gererateStudy()}.`}
        else if (chance >= 21 && chance <= 30) { return `is a refugee who lost its homeworld after ${this.generateDestruction()}.`}
        else if (chance >= 31 && chance <= 40) { return `is seeking its fame and fortune through ${this.gererateStudy()}.`}
        else if (chance >= 41 && chance <= 50) { return `is on a quest for ultimate knowledge of ${this.gererateStudy()}`}
        else if (chance >= 51 && chance <= 60) { return `is seeking some kind of purpose or answer to existance. Wow, won't it be dissapointed.`}
        else if (chance >= 61 && chance <= 70) { return `is on an extended vacation.`}
        else if (chance >= 71 && chance <= 80) { return `is just you know, walking the d-verse, meeting lifeforms... getting into adventures. Like Kane in Kung Fu.`}
        else if (chance >= 81 && chance <= 90) { return `is looking for a... You wouldn't happened to have six fingers on your right hand would you?`}
        else { return `is an absolute mystery, it's intentions are completely unkown`}

    }

    gererateStudy() {
        let chance = random.die(100)
        if (chance >= 1 && chance <= 10) { return `drugs`}
        else if (chance >= 11 && chance <= 20) { return `lifeform trafficing`}
        else if (chance >= 21 && chance <= 30) { return `weapon development`}
        else if (chance >= 31 && chance <= 40) { return `natural resource exploitation`}
        else if (chance >= 41 && chance <= 50) { return `full stack web development`}
        else if (chance >= 51 && chance <= 60) { return `bitcoin`}
        else if (chance >= 61 && chance <= 70) { return `gender studies`}
        else if (chance >= 71 && chance <= 80) { return `alien orifices`}
        else if (chance >= 81 && chance <= 90) { return `imitation arousal cheese`}
        else { return `viagra`}
    }

    generateDestruction() {
        if (chance >= 1 && chance <= 10) { return `it was swallowed by a blackhole`}
        else if (chance >= 11 && chance <= 20) { return `it's life giving star went supernova`}
        else if (chance >= 21 && chance <= 30) { return `an asteroid barreled into its ocean`}
        else if (chance >= 31 && chance <= 40) { return `its species' contributions to the planet's global warming`}
        else if (chance >= 41 && chance <= 50) { return `its species' contributions to the planet's global cooling`}
        else if (chance >= 51 && chance <= 60) { return `it was destroyed due war between its planetary inhabitants`}
        else if (chance >= 61 && chance <= 70) { return `it was destroyed as a result of a galactic war`}
        else if (chance >= 71 && chance <= 80) { return `it was destroyed because of something it did. It doesn't want to talk about it`}
        else if (chance >= 81 && chance <= 90) { return `it was destroyed due to some malfunction of alien technology`}
        else { return `it was destroyed due to lack of knowledge about ${this.gererateStudy()}`}
    }

    generateDisposition() {   
        let chance = random.die(100)
        if (chance >= 1 && chance <= 7) { return `is aloof and always has it's head in the clouds. Very absent minded and forgetful.`}
        else if (chance >= 8 && chance <= 13) { return `is an all around good person. What a loser.`}
        else if (chance >= 14 && chance <= 19) { return `is unbearably arrogant and is always in charge, regardless of what anyone else thinks.`}
        else if (chance >= 20 && chance <= 25) { return `loves to hear itself speak and can't seem to ever shut-up.`}
        else if (chance >= 26 && chance <= 31) { return `is a misserable bastard`}
        else if (chance >= 32 && chance <= 37) { return `is constantly aggravated about something.`}
        else if (chance >= 38 && chance <= 43) { return `is laid back and accepting of others.`}
        else if (chance >= 44 && chance <= 49) { return `is always looking for a way to hustle lifeforms out of something.`}
        else if (chance >= 50 && chance <= 55) { return `is suspicious, spiteful and vindictive.`}
        else if (chance >= 56 && chance <= 61) { return `is quick-tempered and emotional.`}
        else if (chance >= 62 && chance <= 67) { return `always thinks the glass is half empty, and never feels it gets its fair share.`}
        else if (chance >= 68 && chance <= 73) { return `is overprotective of others, especially friends.`}
        else if (chance >= 74 && chance <= 79) { return `is quiet and timid.`}
        else if (chance >= 80 && chance <= 85) { return `is cocky and self-reliant.`}
        else if (chance >= 86 && chance <= 91) { return `tends to rush into situations without thinking it all through.`}
        else { return `is a worry wart and tends to be cautious about everything.`}
    }
